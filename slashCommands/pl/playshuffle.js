...
module.exports = {
  name: "playshuffle", //the command name for the Slash Command
  description: "Shuffle the playlist before playing it", //the command description for Slash Command Overview
  cooldown: 1,
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!	
	//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		...
		{"String": { name: "playlist_name", description: "Playlist name you want to load with shuffled", required: true}}, //here the second array input MUST BE A STRING // TO USE IN THE CODE: interacton.getString("what_ping")
  ],
  run: async (client, interaction, guildData, player, prefix) => {
    try{
      ...
      if (player && channel.id !== player.voiceChannel) {
        ...
        return interaction.reply({embeds: [erme], ephemeral: true});  
      }
      ...
      let tempmsg = await interaction.reply({embeds: [sdfi], ephemeral: true})

      const shuffledQueue = shuffle(fetchList.playlistArray);
      playermanager(client, interaction, shuffledQueue);
      ...
      .setColor(ee.color)
      interaction.editReply({embeds: [tedm]})
    } catch (e) {
